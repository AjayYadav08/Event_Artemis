...
import { Search, MapPin, Users, Rocket, UserPlus } from 'lucide-react';
...
const TeamDiscovery: React.FC<TeamDiscoveryProps> = ({
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-3xl p-8 shadow-xl  -95 ">
       <div className="flex items-center justify-between mb-8">
         <div>
           <h2 className="text-2xl font-black text-slate-900">Discover Teams</h2>
           ...
           <div key={team.id} className="bg-slate-50 border border-slate-100 rounded-[28px] p-6 hover:shadow-xl hover:shadow-slate-200 transition-all group border-b-4 border-b-slate-200 hover:border-b-amber-500">
             <div className="flex items-center justify-between mb-4">
                ...
                <span className={`text-[9px] font-black px-2 py-1 rounded-lg uppercase tracking-widest ${
                   team.status === 'Open' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-200 text-slate-500'
                }`}>
                   {team.status}
                </span>
             </div>
